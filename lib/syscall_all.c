...
int sys_mem_map(int sysno, u_int srcid, u_int srcva, u_int dstid, u_int dstva, u_int perm)
{
    ...
    ret = 0;
        ...
        round_dstva = ROUNDDOWN(dstva, BY2PG);
                ...
                if (srcva >= UTOP || dstva >= UTOP || srcva < 0 || dstva < 0) return -E_UNSPECIFIED;
                if (!(perm & PTE_V)) return -E_INVAL;
                if (envid2env(srcid, &srcenv, 0) < 0 || envid2env(dstid, &dstenv, 0) < 0) return -E_BAD_ENV;
                if ((ppage = page_lookup(srcenv -> env_pgdir, round_srcva, &ppte)) == 0) return -E_UNSPECIFIED;
                if (page_insert(dstenv -> env_pgdir, ppage, round_dstva, perm) < 0) return -E_NO_MEM;
