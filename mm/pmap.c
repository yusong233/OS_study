...
Pde *pgdir_entryp;
    ...
    /* Step 2: If the corresponding page table is not exist and parameter `creat                                                                                                             e`
...
boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    ...
    /* Hint: Use `boot_pgdir_walk` to get the page table entry of virtual addres                                                                                                             s `va`. */
...
/*Overview:
        Given `pgdir`, a pointer to a page directory, pgdir_walk returns a point                                                                                                             er
        to the page table entry (with permission PTE_R|PTE_V) for virtual addres                                                                                                             s 'va'.
...
// Overview:
//      Decrease the `pp_ref` value of Page `*pp`, if `pp_ref` reaches to 0, fre                                                                                                             e this page.
...
page_remove(Pde *pgdir, u_long va)
{
    ...
    /* Step 1: Get the page table entry, and check if the page table entry is va                                                                                                             lid. */
    ...
    *temp = 1000;
    ...
    struct Page *test_pages;
        ...
        for(i=0;i<10;i++) {
                ...
                //printf("0x%x  0x%x\n",&test_pages[i], test_pages[i].pp_link.le                                                                                                             _next);
...
//printf("page_insert");
    ...
    // should be able to map pp2 at BY2PG because pp0 is already allocated for p                                                                                                             age table
